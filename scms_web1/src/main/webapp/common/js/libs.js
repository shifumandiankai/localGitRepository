		...
		initComplete: function(){
			...
			if(add.size() > 0){
				add.click(function(){			
					...
					/*//添加的内容显示头像
					if(config.shown_by_photo){
						config.shown_by_photo(devClone,0,sel_obj);
						return false;
					}
					*/
			...
			if(edit.size() > 0){
				edit.click( function() {
							...
							//编辑的内容要显示头像
							if(config.shown_by_photo){
								config.shown_by_photo(devClone,sel_obj);
								return false;
							}
...
/*	if(input.parent().hasClass('date')) { //datepicker 插件赋值
	...
	/*var form2=div.find("form").eq(1);
	if(form2.length>0){
		var input = form2.find('input[name=' + name + '][type=text]').val(value);
	}*/
...
//按钮
	...
	if(ztreeConf.search!=null){
		controlhtml=controlhtml+'<div id="'+ztreeConf.search+'" class="form-group"><form class="form-horizontal"> <input id="'+ztreeConf.search+'Text" type="text" placeholder="部门名称" class="form-control" style="display: inline-block;width:66%;"> <a  id="'+ztreeConf.search+'Button" href="javascript:" class="btn  btn-default"  tabindex="-1" >搜索</a><input type="hidden" name="searchHide"></form></div>';
	}
	
	if(ztreeConf.search_hide!=null){
		controlhtml=controlhtml+'<input type="hidden" id="search_'+ztreeConf.search_hide+'" >';
	}
	...
	//点击搜索按钮
	$("#"+ztreeConf.search+"Button").on('click',function(){
		var val=$('#'+ztreeConf.search+'Text').val();
			$.ajax({
	              type:'post',
	   		      url: ztreeConf.searchurl,
	              data: {formData:val},
	              success:function(data){
	            	result=JSON.parse(data);
	      			Nodes=result; 
	      			$.fn.zTree.init($("#"+ztreeConf.id),ztreeConf.setting, Nodes);
	          },
	       });
	});
					...
					$.ajax({
						...
						success : function(data) {
							...
							if (result.success) {
						   	...
						   	//更新DataTable数据
								if(ztreeConf.DatatableId!=null){
									$("#" + ztreeConf.DatatableId).DataTable().ajax.reload();
								}
 ...
 * @return {[type]}      [无]
 */
wade.libs.getQueryString =  function(name)
{

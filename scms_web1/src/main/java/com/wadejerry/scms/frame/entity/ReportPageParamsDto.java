...
public ReportPageParamsDto(){
	...
	public static ReportPageParamsDto GetParamsDtoByRequest(HttpServletRequest request) throws ParseException, JSONException {
		...
		ReportPageParamsDto dto = null;
		String val=request.getParameter("columns[0][search][value]");
		if(!val.equals("")){
			//点击查询按钮查询
			dto = new ReportPageParamsDto();
			...
			if(json.has("startTime")&&!json.getString("startTime").equals("")){
				//String str=json.getString("startTime").toString();
				
				dto.setSearchValue0(DateUtil.df.parse(json.getString("startTime").toString()));
			}
			if(json.has("endTime")&&!json.getString("endTime").equals("")){
				//String str=json.getString("endTime").toString();
				dto.setSearchValue1(DateUtil.df.parse(json.getString("endTime").toString()));
			}
			...
			if(json.has("typeId")&&!json.getString("typeId").equals("")){
				dto.setIntValue5(Integer.valueOf(json.getString("typeId")));
			}
			if(json.has("accountType")&&!json.getString("accountType").equals("")){
				dto.setIntValue6(Integer.valueOf(json.getString("accountType")));
			}

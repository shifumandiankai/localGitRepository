...
public class CzReportController {
	...
	public void GetChargeRecordInfoByPage() throws ParseException, JSONException{
		...
		if(paramsDto.getSearchValue0()==null&&paramsDto.getSearchValue1()==null){
			...
			if(listtype!=null){//不是企业管理员，需要权限
					...
					if(listtype.size()>0){
						//有多个权限
							List<CzReportDto> listdto=czReportService.selectDoubleRecordTotal(startTime,endTime,carNumber,listcarType,size,listtype);
							iTotal=listdto.size();
							if(iTotal>0){
								//当天有数据查到
								...
								dtolist=czReportService.selectDoubleListByPage(startTime,endTime,carNumber,listcarType,size,paramsDto.getLength(),paramsDto.getStart(),listtype,order);
	...
	public void ExportDay(@PathVariable("formdata") String formdata) throws ParseException, NumberFormatException, JSONException{
	   ...
	   if((json.has("order1")&&!json.getString("order1").equals(""))&&(json.has("order2")&&!json.getString("order2").equals(""))){
			...
			if(listtype.size()>0){
				...
				for(int i=0;i<listTypeId.size();i++){
					//遍历每一个权限
						...
						List<Map<String,Object>> listMap=czReportService.exportDay(startTime,endTime,list,size,listtype,carNumber,order);
		...
		//是管理员
		data=czReportService.exportDay(startTime,endTime,list,size,listtype,carNumber,order);
	}
		if(data.size()!=0){
	...
	public void selectMonthAllListByPage() throws ParseException, JSONException{
		...
		if(paramsDto.getSearchValue0()==null&&paramsDto.getSearchValue1()==null){
			...
			if(listtype!=null){//不是企业管理员，需要权限
					if(listtype.size()>0){
						//有多个权限
						List<CzReportDto> listdto =czReportService.selectMonthRecordTotal(year,month,carNumber,listcarType,size,startTime,endTime,nowMonth,endmonth,listtype);
						iTotal=listdto.size();
						    if(iTotal>0){
								//当天有数据查到
								if(paramsDto.getLength()==-1){
								...
								if(startTime!=null&&endTime==null){
									...
									dtolist=czReportService.selectMonthListByPage(year,month,carNumber,listcarType,size,paramsDto.getLength(),paramsDto.getStart(),startTime,endTime,nowMonth,endmonth,listtype,order);
									for(CzReportDto temp:dtolist){
										if(insertTime!=null){
	...
	public void ExportMonth(@PathVariable("formdata") String formdata) throws ParseException, JSONException{
		...
		if(listtype!=null){//不是企业管理员，需要权限
				...
				if(listtype.size()>0){
					...
					/*for(int i=0;i<listTypeId.size();i++){
						//遍历每一个权限
							...
							List<Map<String,Object>> listMap=czReportService.exportMonth(startTime,endTime,list,size,listtype,carNumber,nowMonth,year,order);
						    for(Map temp:listMap){
							data.add(temp);
						    }
			...
			//是管理员
			data=czReportService.exportMonth(startTime,endTime,list,size,listtype,carNumber,nowMonth,year,order);
		}
	...
	public void selectYearAllListByPage() throws ParseException, JSONException{
		...
		if(paramsDto.getSearchValue0()==null&&paramsDto.getSearchValue1()==null){
			...
			if(listtype!=null){//不是企业管理员，需要权限
					...
					if(listtype.size()>0){
						//有多个权限
						    List<CzReportDto> listdto =czReportService.selectYearRecordTotal(startyear,carNumber,listcarType,size,startTime,endTime,endyear,listtype,nowYear);
						    iTotal=listdto.size();
						    if(iTotal>0){
								//当天有数据查到
								if(paramsDto.getLength()==-1){
								...
								if(startTime!=null&&endTime==null){
									...
									dtolist=czReportService.selectYearListByPage(startyear,endyear,carNumber,listcarType,size,paramsDto.getLength(),paramsDto.getStart(),startTime,endTime,nowYear,listtype,order);
									for(CzReportDto temp:dtolist){
										if(insertTime!=null){
	...
	public void ExportYear(@PathVariable("formdata") String formdata) throws ParseException, JSONException{
		...
		if(listtype!=null){//不是企业管理员，需要权限
				if(listtype.size()>0){
					//有多个权限
							List<Map<String,Object>> listMap=czReportService.exportYear(startyear,carNumber,list,size,startTime,endTime,endyear,listtype,nowYear,order);
						    for(Map temp:listMap){
							data.add(temp);
						    }	
				}
			...
			//是管理员
			data=czReportService.exportYear(startyear,carNumber,list,size,startTime,endTime,endyear,listtype,nowYear,order);
		}
		if(data.size()!=0){
			for(int i=0;i<data.size();i++){
				String time=null;

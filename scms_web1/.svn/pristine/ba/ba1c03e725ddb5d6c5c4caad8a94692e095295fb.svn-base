	...
	*/
	function Timepicker() {
		...
		this._defaults = { // Global defaults for all the datetime picker instances
			...
			showMillisec: false,
			showTimezone: false,
			showDate: true,//设置是否显示日期----zengxiao,2013-10-21 支持月报表不显示日期
			showTime: true,
			stepHour: 1,
			stepMinute: 1,
			stepSecond: 1,
		...
		*/
		_injectTimePicker: function() {
			var $dp = this.inst.dpDiv,
				...
				gridSize = {},
				size = null;
			//添加隐藏日期功能-----zengxiao 2013.10.21
			if(o.showDate===false){
				if(!$dp.hasClass("no-date-picker")){
					$dp.addClass('no-date-picker');
				}
			}else{
				$dp.removeClass('no-date-picker');
			}
			//在这里增加清除按钮---------zengxiao 2013.3.28
			if(this._defaults.showClear && $dp.find('div.ui-datepicker-buttonpane button.clear-value').length === 0){
				$dp.find('div.ui-datepicker-buttonpane').append("<button class='clear-value ui-state-default ui-priority-primary ui-corner-all'>清除</button>");
				var $inputDate = this.$input;
				$dp.find('div.ui-datepicker-buttonpane button.clear-value').on('click',function(){
					$($inputDate).val("");
				});
			}
		...
		*/
		_updateDateTime: function(dp_inst) {
			...
			*/
			...
			formattedDateTime = formattedDateTime.replace(/(^\s*)|(\s*$)/g,"");

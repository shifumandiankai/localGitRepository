<%@ page contentType="text/html; charset=UTF-8"%>
<link rel="stylesheet" href="/plugins/datatables/dataTables.bootstrap.css">
<link rel="stylesheet" href="/pages/pfm/carmanage/css/jquery.shCircleLoader.css">
<link rel="stylesheet" type="text/css" href="/pages/pfm/carmanage/css/car_manage.css" >
<link rel="stylesheet" href="/pages/onecard/personnel/fingerprint.css?<%=System.currentTimeMillis() %>">

		<!-- Content Wrapper. Contains page content -->
<div class="content-wrapper">
	 <section class="content-header">
		<h1>
			人员管理界面
		</h1>
		<ol class="breadcrumb">
			<li><a href="/index.do"><i class="fa fa-dashboard"></i> 主页</a></li>
			<li class="active">人员管理</li>
		</ol>
	</section>
	<!-- Main content -->
	<section class="content">
	<div class="box">
	<!-- 	<div class="box-header">
			<h3 class="box-title"></h3>
		</div> -->
		<div class="box-body">
			<div>
				<div id="left">
					
					<ul id="tree" class="ztree" style="margin-left:10px;"></ul>
	
				</div>

				<div id="right">
                      <!-- <input type="hidden" name ="searchValue"> -->
					<div class="nav-tabs-custom">
						<ul class="nav nav-tabs">
							<li class="active"><a href="#carinfo" data-toggle="tab">人员管理</a></li>
						
						</ul>
						<div class="tab-content">

							<div class="tab-pane active" id="first">
								<table id="personnelexample" class="table table-bordered table-striped">
								
									<thead>
										<tr>
											<th></th>
											<th></th> 
											<th></th>
											<th></th>
											<th></th>
											<th></th>
											<th></th>
											<th></th>
											<th></th>
											<th></th>
											<th></th>
											<th></th>
											<th></th>
											<th></th>
											<th></th>
											<th></th>
											<th></th>
											<th></th>
											<th></th>
											<th></th>
											<th>编号</th>
											<th>姓名</th>
											<th>性别</th>
											<th>证件号</th>
											<th>证件类型</th>
											<th>拼音代码</th>
											<th>联系电话</th>
											
											
											
										
										</tr>
									</thead>
									<tbody>
									</tbody>
									<tfoot>
									</tfoot>
								</table>
							</div>
							
							<!-- rule -->
						</div>
						<!-- /.tabcontent -->
					</div>
					<!-- /.nav-tabs-custom -->
				</div>
				<!-- /.right -->


			</div>
			<!--/. container -->

		</div>
		<!-- /.box-body -->
	</div>
	<!-- /.box -->
	</section>
</div>
	<%@ include file="personnel_adddeptdialog.jsp"%>
	<%@ include file="batchcardDialog.jsp"%>
	<%@ include file="changedeptDialog.jsp"%>
	<%@ include file="exportDialog.jsp"%>
	<%@ include file="importDialog.jsp"%>
	<%@ include file="../Card/cardDialog.jsp"%>
	<%@ include file="../Card/swapcardDialog.jsp"%>
	<%@ include file="../Card/bindcardDialog.jsp"%>
	<%@ include file="../Card/fingersqDialog.jsp"%>
	<%@ include file="../Card/carnumberDialog.jsp"%>
	 <%@ include file="loadDialog.jsp"%> 
	<%@ include file="bimcateDialog.jsp"%>
	<%@ include file="personnel_addpersondialog.jsp"%>
	<script src="/pages/onecard/personnel/personnel.js" type="text/javascript"></script>
	<script src="/plugins/My97DatePicker/WdatePicker.js"></script>
		